...
同じユーザーでパスワード認証に5回連続して失敗すると、そのユーザーを15分間ロックします。ロック中は正しいパスワードでもHTTP `429 Too Many Requests`を返し、解除までの秒数を`Retry-After`ヘッダーに設定します。15分経過後、またはロック前にログインへ成功すると失敗回数をリセットします。
...
これらはユーザー名単位の制限です。存在しないユーザー名や複数ユーザーを使った攻撃、送信元IP単位の大量アクセスは防げません。本番では、IP単位のレート制限と長時間枠のSMS送信上限も追加してください。
