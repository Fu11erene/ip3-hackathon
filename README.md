...
検証環境では、正常送信用に`09001111101`〜`09001111104`、エラー確認用に`09001111201`〜`09001111204`と`09002222001`を使用します。配布資料では先頭の`0`が省略されていますが、APIへ送る`to`には、`090`から始まる11桁の国内形式を指定します。デモユーザーの電話番号は`09001111101`を初期値にしています。
...
検証環境で実端末にSMSが届かない場合は、ローカルの`.env`で`LOG_OTP_CODE=true`にすると、APIログからOTPを確認できます。

```bash
docker compose logs -f api
```

ログには`開発用OTP: username=test challenge_id=... code=123456`の形式で出力されます。OTPは認証情報のため、本番環境では必ず`LOG_OTP_CODE=false`にしてください。
