...
sequenceDiagram
    ...
    A->>A: ロック状態を確認し、bcryptでパスワード照合
    A->>D: 失敗回数を更新、または成功時にリセット
    A->>A: 前回のSMS受付から60秒以上か確認
    A->>D: OTPチャレンジを作成
...
オンラインでパスワードを繰り返し試す総当たり攻撃を抑えるため、同じユーザーで認証に5回連続して失敗すると、そのユーザーを15分間ロックします。ロック中は正しいパスワードでもHTTP `429 Too Many Requests`を返し、解除までの秒数を`Retry-After`ヘッダーに設定します。15分経過後、またはロック前にログインへ成功すると失敗回数をリセットします。

試行回数とロック期限はPostgreSQLのユーザーレコードに保存するため、APIを再起動しても制限は継続します。上限とロック時間は、環境変数`LOGIN_MAX_ATTEMPTS`と`LOGIN_LOCK_MINUTES`で変更できます。

パスワードは平文で保存せず、ランダムなsaltを含むbcryptハッシュとして保存します。DBが漏えいした場合のオフライン解析を難しくしますが、推測されやすいパスワードを安全にするものではありません。そのため、後述のパスワードポリシーも併用します。

現在のロックはユーザー名単位です。存在しないユーザー名への試行、複数のユーザー名を使うパスワードスプレー攻撃、送信元IPを変える攻撃は防げません。本番では、リバースプロキシやAPI GatewayでIP単位のレート制限を追加し、認証失敗の監視・通知も行ってください。

### SMS再送の制限

パスワード認証に成功しても、前回のSMS送信受付から60秒未満の場合は新しいSMSを送りません。この場合もHTTP `429`と`Retry-After`を返します。SMS送信APIへの受付が失敗した場合は送信時刻を元に戻すため、直ちに再試行できます。

再送間隔は環境変数`OTP_RESEND_INTERVAL_SECONDS`で変更できます。この制限もユーザー名単位のため、本番ではIP単位のレート制限と長時間枠のSMS送信上限を追加してください。

### パスワードポリシー

新規ユーザー登録時のパスワードには、次の条件を適用します。条件を満たさない場合、`POST /auth/register`はHTTP `422 Unprocessable Content`と不足している条件を返します。

- 12文字以上64文字以下
- UTF-8で72バイト以下
- 英大文字、英小文字、数字、記号を各1文字以上含む
- 空白を含めない
- 記号を除いて比較したときにユーザー名を含めない
- `password`など、登録済みの代表的な弱いパスワードと一致しない

このポリシーは新規登録時に適用します。すでにDBへ保存されているパスワードは、ログイン時に再検証しません。

複雑さの条件だけでは、使い回しや既知の漏えいパスワードを十分に検出できません。本番では、登録・変更時に漏えい済みパスワードの照合を追加し、既存ユーザーにはパスワード変更の導線を用意してください。
...
デモ用アカウント`demo / Hackathon1!Safe`は`07020213632`を使用します。実端末での確認では、このアカウントでログインします。

検証環境が正常送信用として用意している番号を使う場合は、別のユーザーを登録します。

```bash
curl -X POST http://localhost:8080/api/auth/register \
  -H "Content-Type: application/json" \
  -d '{"username":"delivery-demo","password":"Hackathon1!Safe","phone_number":"09001111101"}'
```

`delivery-demo / Hackathon1!Safe`でログインし、OTP画面が`pending`から`delivered`へ変わることを確認します。検証用番号では実端末に届かない場合があるため、OTPはAPIログで確認できます。

### 失敗系

失敗番号を持つユーザーは自動作成されません。次の例では`error-demo / Hackathon1!Safe`を登録します。

```bash
curl -X POST http://localhost:8080/api/auth/register \
  -H "Content-Type: application/json" \
  -d '{"username":"error-demo","password":"Hackathon1!Safe","phone_number":"09001111201"}'
```

登録後に`error-demo / Hackathon1!Safe`でログインします。状態が`pending`から`failed`へ変わり、配信エラーの内容が表示されれば確認完了です。失敗確定まで1分以上かかる場合があります。
...
| `POST` | `/auth/register` | 不要 | パスワードポリシーを検証してユーザー登録 |
...
アクセストークンの有効期限は既定で60分です。認証エラーは`401`、存在しないOTPチャレンジは`404`、ユーザー名の重複は`409`、パスワードポリシー違反は`422`、ログインロック中またはSMS再送間隔内は`429`、CPaaS NOWへの送信受付に失敗した場合は`502`を返します。入力スキーマや全レスポンスはOpenAPI画面で確認してください。
...
- ハードコードされたデモ用アカウントの自動作成を削除する
- PostgreSQLとFastAPIのホスト向けポート公開をやめる
- CORSの許可元を限定する
- 公開ユーザー登録を無効化するか、管理者だけに許可する
- `/items`を削除するか、必要な認可を追加する
- IP単位のログイン制限と、長時間枠のSMS送信上限を追加する
- OTP検証と配信状態確認へレート制限を追加する
- ユーザー名と電話番号の入力制約を追加する
...
### `demo / Hackathon1!Safe`でログインできない
...
レスポンスの`detail`と`Retry-After`ヘッダーを確認します。パスワードを5回連続で間違えた場合は最大15分、SMS送信後すぐに再ログインした場合は最大60秒待ってから再試行してください。
